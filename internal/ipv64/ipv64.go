...
func NewClient(token string) *Client {
	klog.V(6).Infof("create new ipv64 client")
	if client == (Client{}) {
		client = Client{
			ApiUrl: ApiUrl,
			Token:  token,
...
func (c *Client) AddDNSRecord(subdomain string, praefix string, content string, recordType string) error {
	...
	klog.V(6).Infof("call function AddDNSRecord: subdomain=%s, praefix=%s, content=%s, recordType=%s")
	...
	klog.V(6).Infof("Added record %s.%s", praefix, subdomain)
...
func (c *Client) DeleteDNSRecord(subdomain string, praefix string, content string, recordType string) error {
	...
	klog.V(6).Infof("call function DeleteDNSRecord: subdomain=%s, praefix=%s, content=%s, recordType=%s")
	...
	klog.V(6).Infof("Deleted record %s.%s", praefix, subdomain)
